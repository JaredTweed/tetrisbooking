...
function updateUIBasedOnUser(user) {
  ...
  if (user) {
    ...
    if (authLinks) authLinks.innerHTML = `<a href="/logout">Logout</a>`;
    ...
    if (authLinks) authLinks.innerHTML = `<a href="/auth/google">Login with Google</a>`;
  }
}
...
function checkAvailability() {
  ...
  fetch(`/api/calendar/${route}/availability`)
    .then((res) => res.json())
    .then((data) => {
      if (data.message) {
...
function updateAvailableTimesForDate(dateStr) {
  ...
  fetch(`/api/calendar/${route}/booked?date=${dateStr}`)
    .then((res) => res.json())
    .then((data) => {
...
async function precomputeFullyBookedDays(route, windowDays) {
  ...
  try {
    const res = await fetch(
      `/api/calendar/${route}/booked-range?start=${startStr}&end=${endStr}`
    );
...
function cancelAppointment(appointmentId) {
  ...
  fetch(`/api/my-appointments/${appointmentId}`, {
    method: "DELETE",
  })
    .then((res) => res.json())
    .then((data) => {
...
function fetchAppointmentsForDate(dateStr) {
  fetch(`/api/calendar/${calendarRouteEl.value.trim()}/booked?date=${dateStr}`)
    .then((res) => {
      return res.json();
    })
    .then((data) => {
